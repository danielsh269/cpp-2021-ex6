...
BOOST_AUTO_TEST_CASE(square_brackets_operator)
{
  ...
  try
    {
      v1[13];
      BOOST_TEST (true, "operator[] didn't throw exceptions.\n");
    }
  catch (const std::exception &e)
    { BOOST_TEST (false, "operator[] shouldn't throw exceptions.\n"); }
  ...
  for (int i = 0; i < 8; ++i)
    {
      ...
      BOOST_TEST (v.capacity () == (size_t) (3 * arr_len) / 2);
      BOOST_TEST (v.data () == dynamic_vec);
      pos = v.erase (v.begin () + 10);
      BOOST_TEST (pos == v.begin () + 10);
    }
